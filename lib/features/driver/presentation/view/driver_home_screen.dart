...
class _DriverHomeScreenState extends State<DriverHomeScreen>
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.account_balance_wallet, color: primarySage, size: 22),
                  ),
              ...
              const Text(
                "Enter the deposit amount passengers will pay before boarding",
                    ...
                    child: SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          ...
                          context.read<DriverCubit>().goOnline(depositAmount: deposit);
                        },
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  validator: (value) {
                    if (value == null || value.trim().isEmpty) return "Please enter a number";
                    final number = int.tryParse(value.trim());
                    if (number == null) return "Please enter a valid number";
                    if (number <= 0) return "Must be greater than 0";
                    if (number > 14) return "Must not exceed 14";
                    return null;
                  },
                ),
                const SizedBox(height: 24),
        ...
        child: Container(
          ...
          child: const Row(
            children: [
              ...
              Text(
                "You are now online!",
                style: TextStyle(color: successGreen, fontSize: 14, fontWeight: FontWeight.w500),
              ),
            ],
          ),
        ),
  ...
  Widget _buildOfflineStatusCard(BuildContext context, DriverState state, bool isSmallScreen) {
    return Container(
      ...
      child: Column(
        children: [
          Text("Go Online", style: TextStyle(color: darkText, fontSize: isSmallScreen ? 20 : 24, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          const Text("Start accepting ride requests", style: TextStyle(color: Colors.grey, fontSize: 14)),
          SizedBox(height: isSmallScreen ? 16 : 24),
          SizedBox(
            width: double.infinity,
            height: 54,
            child: ElevatedButton(
              onPressed: state.isLoading ? null : () => _showDepositDialog(context),
              style: ElevatedButton.styleFrom(
                backgroundColor: primarySage,
                elevation: 0,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
              ...
              child: state.isLoading
                ...
                children: [
                  ...
                  SizedBox(width: 8),
                  Text("Go Online", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                ],
              ),
            ),
          ),
  ...
  Widget _buildOnlineStatusCard(BuildContext context, DriverState state, bool isSmallScreen) {
    return Container(
      ...
      child: Column(
        children: [
          Text("You're Online", style: TextStyle(color: darkText, fontSize: isSmallScreen ? 20 : 24, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          const Text("Ready to accept rides", style: TextStyle(color: Colors.grey, fontSize: 14)),
          SizedBox(height: isSmallScreen ? 16 : 24),
          SizedBox(
            width: double.infinity,
            height: 54,
            child: ElevatedButton(
              ...
              child: state.isLoading
                ...
                children: [
                  Icon(Icons.power_settings_new, color: Colors.white),
                  SizedBox(width: 8),
                  Text("Go Offline", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                ],
              ),
            ),
          ),
  ...
  Widget _buildQueuePositionCard(DriverState state, bool isSmallScreen) {
    ...
    return Container(
      width: double.infinity,
      padding: EdgeInsets.symmetric(vertical: isSmallScreen ? 24 : 32, horizontal: isSmallScreen ? 16 : 24),
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(24.0),
        boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.05), blurRadius: 15, offset: const Offset(0, 5))],
      ...
      child: Column(
        children: [
          Icon(isReady ? Icons.check_circle_outline : Icons.group_outlined, color: isReady ? successGreen : Colors.grey, size: isSmallScreen ? 32 : 40),
          SizedBox(height: isSmallScreen ? 8 : 12),
          Text(
            isReady ? "Your vehicle is ready for boarding" : "Queue Position",
            style: TextStyle(color: isReady ? successGreen : Colors.grey, fontSize: 14, fontWeight: isReady ? FontWeight.bold : FontWeight.normal),
          ),
          const SizedBox(height: 4),
          Text("#${state.queuePosition == 0 ? 4 : state.queuePosition}",
              style: TextStyle(color: primarySage, fontSize: isSmallScreen ? 36 : 48, fontWeight: FontWeight.bold)),
          const SizedBox(height: 4),
          const Text("in Cairo Central Station", style: TextStyle(color: Colors.grey, fontSize: 14)),
          ...
          if (isReady)
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: primarySage, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
                child: const Text("Start Boarding", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
              ),
            )
          else
            Container(
              ...
              child: const Text("Estimated wait: ~32 minutes", style: TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.w500)),
            ),
        ],
      ),
    );
  ...
  Widget _buildPerformanceCard(DriverState state, bool isSmallScreen) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text("Today's Performance", style: TextStyle(color: darkText, fontSize: isSmallScreen ? 16 : 18, fontWeight: FontWeight.bold)),
          SizedBox(height: isSmallScreen ? 12 : 20),
          GridView.count(
            crossAxisCount: 2,
            shrinkWrap: true,
            ...
            children: [
              _buildStatItem(icon: Icons.trending_up, value: state.completedTrips.toString(), label: "Completed Trips", isSmallScreen: isSmallScreen),
              _buildStatItem(icon: Icons.account_balance_wallet_outlined, value: "${state.totalEarnings.toStringAsFixed(0)} EGP", label: "Total Earnings", isSmallScreen: isSmallScreen),
              _buildStatItem(icon: Icons.access_time, value: DurationFormatter.fromMinutes(state.totalActiveMinutes), label: "Active Hours", isSmallScreen: isSmallScreen),
              _buildStatItem(icon: Icons.group_outlined, value: "${state.avgRating}★", label: "Avg Rating", isSmallScreen: isSmallScreen),
            ],
          ),
        ],
      ),
  ...
  Widget _buildStatItem({required IconData icon, required String value, required String label, required bool isSmallScreen}) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: isSmallScreen ? 8 : 12),
          Text(value, style: TextStyle(color: darkText, fontSize: isSmallScreen ? 14 : 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 4),
        ...
        Expanded(
          child: _buildActionButton(
            ...
            onPressed: state.isOnline ? () {
              showDialog(
                context: context,
                builder: (_) => BlocProvider.value(value: context.read<DriverCubit>(), child: const QrScannerDialog()),
              );
            } : null,
  ...
  Widget _buildActionButton({required String label, required VoidCallback? onPressed, required bool isSmallScreen}) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isEnabled ? Colors.grey.withValues(alpha: 0.2) : Colors.grey.withValues(alpha: 0.1)),
      ),
      child: TextButton(
        onPressed: onPressed,
        style: TextButton.styleFrom(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16.0))),
        child: Text(
          label,
          style: TextStyle(color: isEnabled ? darkText : Colors.grey.withValues(alpha: 0.5), fontWeight: FontWeight.bold, fontSize: isSmallScreen ? 13 : 15),
        ),
      ),
    );
  }
